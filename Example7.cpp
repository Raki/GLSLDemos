...
auto showImgDiff = false;
auto renderFbo1 = true;
auto renderFbo2 = true;

std::shared_ptr<Mesh> fsQuad;
std::shared_ptr<GlslProgram> aspectProgram,blendProgram,diffProgram,
                            blurProgram;
std::shared_ptr<FrameBuffer> fbo1, fbo2,blurFbo;
...
const char* images[] = { "img/landscape.jpg","img/portrait.jpg","img/JohnGreen7.png", "img/Lenna.png","img/prerendered.jpg","img/prerendered.original.jpg","img/sample_shapes.png"};
const std::vector<std::string> sampleTextures{"img/landscape.jpg","img/portrait.jpg","img/JohnGreen7.png", "img/Lenna.png" ,"img/prerendered.jpg","img/prerendered.original.jpg","img/sample_shapes.png"};

GLuint normalColor;
GLuint textureColor;
GLuint blendColor,diffColor;
#pragma endregion 
...
void createWindow()
{
    ...
    window = glfwCreateWindow(WIN_WIDTH, WIN_HEIGHT, "Example 6", NULL, NULL);
...
void initGL()
{
    ...
    blendProgram = std::make_shared<GlslProgram>(Utility::readFileContents("shaders/vAspect.glsl"), Utility::readFileContents("shaders/fAdditive.glsl"));
    blurProgram = std::make_shared<GlslProgram>(Utility::readFileContents("shaders/vBlur.glsl"), Utility::readFileContents("shaders/fBlur.glsl"));
    ...
    blendColor = glGetSubroutineIndex(blendProgram->programID, GL_FRAGMENT_SHADER, "getBlendColor");
    diffColor = glGetSubroutineIndex(blendProgram->programID, GL_FRAGMENT_SHADER, "getDiffColor");
    ...
    blurFbo = GLUtility::makeFbo(WIN_WIDTH, WIN_HEIGHT, 0);
...
void renderFrame()
{
    ...
    TextureInfo tInfo;
    tInfo.aspectScale = glm::vec2(1);
    tInfo.dim = glm::vec2(fbo1->width,fbo1->height);
    tInfo.texture = fbo1->color;
    applyBlur(blurFbo, tInfo);
    ...
    glBindTexture(GL_TEXTURE_2D, blurFbo->color);
    blendProgram->bindAllUniforms();
...
void applyBlur(std::shared_ptr<GLUtility::FrameBuffer> fboContainer, TextureInfo texInfo)
{
    ...
    blurProgram->setInt("sampler", 0);
    blurProgram->setFloat("r", blurRadius);
    blurProgram->setVec2f("scaleFactrs", texInfo.aspectScale);
    blurProgram->setVec2f("imgDim", texInfo.dim);
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, texInfo.texture);
    blurProgram->bindAllUniforms();
